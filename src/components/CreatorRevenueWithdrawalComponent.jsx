...
const CreatorRevenueWithdrawalComponent = () => {
  ...
  const loadRaffleInfo = async (raffleAddress) => {
    ...
    try {
      ...
      const stateNames = [
        'Pending', 'Active', 'Ended', 'Drawing', 'Completed', 'Cancelled'
      ];
  ...
  const handleWithdrawRevenue = async () => {
    ...
    const validStates = ['Completed', 'Cancelled'];
    if (!validStates.includes(raffleData.raffleState)) {
      alert(`Revenue can only be withdrawn from completed or cancelled raffles. Current state: ${raffleData.raffleState}`);
      return;
    }
  ...
  const canWithdraw = raffleData.isCreator && 
                     ...
                     ['Completed', 'Cancelled'].includes(raffleData.raffleState);

  return (
    <div className="bg-card border border-border rounded-lg p-6">
      <div className="flex items-center gap-2 mb-6">
        <DollarSign className="h-5 w-5" />
        <h3 className="text-lg font-semibold">Withdraw Creator Revenue</h3>
      </div>
      ...
      <div className="space-y-6">
        ...
        {raffleData.address && raffleData.raffleState !== 'unknown' && (
          <div className="p-4 bg-muted rounded-lg">
            ...
            {raffleData.isCreator && !['Completed', 'Cancelled'].includes(raffleData.raffleState) && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md flex items-center gap-2">
                <AlertCircle className="h-4 w-4 text-blue-600" />
                <span className="text-sm text-blue-800">
                  Revenue can only be withdrawn from completed or cancelled raffles.
                </span>
              </div>
            )}
        ...
        <div className="p-4 bg-muted/50 rounded-lg">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            ...
            <p className="mt-2 font-medium">
              Note: Revenue can only be withdrawn from completed or cancelled raffles.
            </p>
          </div>
        </div>

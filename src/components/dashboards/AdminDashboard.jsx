...
import { Shield, Settings, DollarSign, Clock, Users, Package, Pause } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchFees = async () => {
      ...
      try {
        ...
        const [protocolFee, creationFee, ticketPrice] = await Promise.all([
        ...
        console.log('Fees: Received data:', { protocolFee, creationFee, ticketPrice });
        setFormData({
...
const TransferOwnershipConfiguration = () => {
  ...
  const handleTransfer = async () => {
    ...
    try {
      const result = await executeTransaction(
        contracts.raffleManager.transferOwnership,
        newOwner
      );
      if (result.success) {
      ...
      setError('Error transferring ownership: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ConfigSection title="Transfer Ownership" icon={Shield}>
      <div className="space-y-2">
...
const SetCreatorTokenConfiguration = () => {
  const { contracts, executeTransaction } = useContract();
  const [fetching, setFetching] = useState(true);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [currentToken, setCurrentToken] = useState('');
  const [currentMinBalance, setCurrentMinBalance] = useState('');
  ...
  useEffect(() => {
    const fetchCreatorToken = async () => {
      ...
      try {
        const [tokenAddr, minBal, gated] = await Promise.all([
          contracts.raffleManager.creatorToken(),
          contracts.raffleManager.minCreatorTokenBalance(),
          contracts.raffleManager.isCreatorTokenGated()
        ]);
        setCurrentToken(tokenAddr);
        setCurrentMinBalance(minBal?.toString?.() ?? '');
        setIsGated(gated);
        setError(null);
      } catch (e) {
        setError('Failed to fetch creator token config.');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSetToken = async () => {
    ...
    if (!minBalance || isNaN(minBalance) || Number(minBalance) < 0) {
      setError('Invalid min balance');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const result = await executeTransaction(
        contracts.raffleManager.setCreatorToken,
        token,
        minBalance
      );
      if (result.success) {
        alert('Creator token and min balance set successfully!');
        setToken('');
        setMinBalance('');
        // Refetch current values
        const [tokenAddr, minBal] = await Promise.all([
          contracts.raffleManager.creatorToken(),
          contracts.raffleManager.minCreatorTokenBalance()
        ]);
        setCurrentToken(tokenAddr);
      ...
      setError('Error setting creator token: ' + e.message);
    } finally {
      setLoading(false);
    }
  };

  const handleToggleGated = async () => {
    if (!contracts.raffleManager) {
      setError('RaffleManager contract not configured');
      return;
    }
    ...
    try {
      const result = await executeTransaction(
        contracts.raffleManager.toggleCreatorTokenGated,
        !isGated
      );
      if (result.success) {
        setIsGated(!isGated);
      ...
      setError('Error toggling creator token gating: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ConfigSection title="Creator Token Gating" icon={Settings}>
      <div className="space-y-2">
        {fetching ? (
          <div className="text-sm text-muted-foreground">Loading creator token config...</div>
        ) : (
          <>
            <div className="text-sm">
              <span className="font-medium">Current Token:</span> {currentToken}
            </div>
            <div className="text-sm">
              <span className="font-medium">Current Min Balance:</span> {currentMinBalance}
            </div>
            <div className="text-sm">
              <span className="font-medium">Gating Enabled:</span> {isGated ? <span className="text-green-600 font-semibold">Yes</span> : <span className="text-red-600 font-semibold">No</span>}
            </div>
          </>
        )}
        <div>
          <label className="block text-sm font-medium mb-1">New Token Address</label>
          <input
            type="text"
            value={token}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">New Min Balance</label>
          <input
            type="number"
            min="0"
            value={minBalance}
        ...
        <button
          ...
          disabled={loading || !ethers.utils.isAddress(token) || !minBalance || isNaN(minBalance) || Number(minBalance) < 0}
          className="w-full bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90 transition-colors disabled:opacity-50"
        >
          {loading ? 'Setting...' : 'Set Creator Token & Min Balance'}
        </button>
        <button
          onClick={handleToggleGated}
          disabled={loading || fetching}
          className={`w-full mt-2 px-4 py-2 rounded-md transition-colors disabled:opacity-50 ${isGated ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-green-600 text-white hover:bg-green-700'}`}
        >
          {loading ? (isGated ? 'Disabling...' : 'Enabling...') : (isGated ? 'Disable Gating' : 'Enable Gating')}
        </button>
      </div>
    </ConfigSection>
  );
...
const PauseRaffleCreationConfiguration = () => {
  const { contracts, executeTransaction } = useContract();
  ...
  useEffect(() => {
    const fetchPaused = async () => {
      ...
      try {
        const isPaused = await contracts.raffleManager.creationPaused();
        setPaused(isPaused);
        setError(null);
      } catch (e) {
        setError('Failed to fetch pause status.');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleTogglePause = async () => {
    ...
    try {
      const result = await executeTransaction(
        contracts.raffleManager.pauseRaffleCreation,
        !paused
      );
      if (result.success) {
        setPaused(!paused);
      ...
      setError('Error toggling pause: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ConfigSection title="Pause Raffle Creation" icon={Pause || Settings}>
      <div className="space-y-2">
        {fetching ? (
        ...
        <button
          ...
          disabled={loading || fetching}
          className={`w-full px-4 py-2 rounded-md transition-colors disabled:opacity-50 ${paused ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-red-600 text-white hover:bg-red-700'}`}
        >
          {loading ? (paused ? 'Unpausing...' : 'Pausing...') : (paused ? 'Unpause Raffle Creation' : 'Pause Raffle Creation')}
        </button>
      </div>
    </ConfigSection>
  );
...
const AdminDashboard = () => {
  ...
  const contractsConfigured = contracts.raffleManager && contracts.revenueManager;
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <SetCreatorTokenConfiguration />
        <PauseRaffleCreationConfiguration />
      </div>
